...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        new_strides = tuple(
            self.strides[i] if self.shape[i] != 1 else 0 for i in range(len(self.shape))
        )
        return self.as_strided(new_shape, new_strides)
        ### END YOUR SOLUTION
