...
__global__ void ScatterSetitemKernel(scalar_t *a, scalar_t val, size_t size,
                                     CudaVec shape, CudaVec strides,
                                     size_t offset) {
  /**
   * The CUDA kernel for the scatter setitem operation.  This should effectively
   * map a single entry in the non-compact input a, to the corresponding item (at
   * location gid) in the compact array out.
   *
   * Args:
   *   a: CUDA pointer to a array
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray *out,
  ...
  ScatterSetitemKernel<<<dim.grid, dim.block>>>(
...
__global__ void ScalarPowerKernel(const scalar_t *a, scalar_t val, scalar_t *out,
                                  size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
...
__global__ void ScalarMaximumKernel(const scalar_t *a, scalar_t val, scalar_t *out,
                                    size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
...
void ScalarEq(const CudaArray &a, scalar_t val, CudaArray *out) {
  /**
   * Equal a CUDA array and a scalar value.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseGe(const CudaArray &a, const CudaArray &b, CudaArray *out) {
  /**
   * Greater or equal two CUDA array
   */
  CudaDims dim = CudaOneDim(out->size);
...
void ScalarGe(const CudaArray &a, scalar_t val, CudaArray *out) {
  /**
   * Greater or equal a CUDA array and a scalar value.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void Matmul(const CudaArray &a, const CudaArray &b, CudaArray *out, uint32_t M,
  ...
  MatmulKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, P, N);
    /// END YOUR SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  if (gid < size) {
    ...
    for (size_t i = 1; i < reduce_size; ++i) {
      max = fmax(max, a[gid * reduce_size + i]);
    }
    out[gid] = max;
  }
}
