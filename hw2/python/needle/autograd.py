...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        sum_of_partial_grads = node_to_output_grads_list[node][0]
        for i in range(1, len(node_to_output_grads_list[node])):
            sum_of_partial_grads += node_to_output_grads_list[node][i]
        
        node.grad = sum_of_partial_grads

        if node.op is not None:
            partial_grads = node.op.gradient_as_tuple(sum_of_partial_grads, node)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:
        ...
        topo_sort_list.extend(topo_order)
